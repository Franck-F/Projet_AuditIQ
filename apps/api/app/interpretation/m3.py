...
from app.interpretation._findings import finding_from_m3
...
def _deterministic_recommendations(
    result: M3Result, sector: str | None
) -> list[RecommendationOut]:
    return build_recommendations(finding_from_m3(result, sector))


def _fallback(
    result: M3Result, *, sector: str | None = None, degraded: bool = False
) -> InterpretationOut:
    verdicts = {
...
async def interpret_m3(
    result: M3Result, *, provider: LLMProvider | None, sector: str | None = None
) -> InterpretationOut:
    if provider is None:
