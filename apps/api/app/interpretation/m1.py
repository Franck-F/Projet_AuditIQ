...
from app.interpretation._findings import finding_from_m1
from app.interpretation._json import loads_lenient
from app.interpretation._recommendations import merge_llm_text
from app.interpretation._recommendations import skeleton_json as _skeleton_json
...
def _deterministic_recommendations(
    result: M1Result, sector: str | None
) -> list[RecommendationOut]:
    return build_recommendations(finding_from_m1(result, sector))


def _fallback(
    result: M1Result, *, sector: str | None = None, degraded: bool = False
) -> InterpretationOut:
...
async def interpret_m1(
    result: M1Result, *, provider: LLMProvider | None, sector: str | None = None
) -> InterpretationOut:
    if provider is None:
