...
def run_m1(df: pd.DataFrame, config: M1Config) -> M1Result:
    ...
    if intersectional is not None:
        pairwise_list.append(intersectional)

    agg_verdict = (intersectional.verdict if intersectional is not None
                   else verdict)
    agg_risk = (intersectional.risk_score if intersectional is not None
                else score)
