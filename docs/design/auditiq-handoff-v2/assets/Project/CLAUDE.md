...
| Style | **Tailwind CSS** + tokens CSS variables (cf. `tokens.css`) | Reproduction fidèle de la maquette |
...
│   ├── tokens/               # CSS vars + Tailwind preset (depuis tokens.css)
...
- **Tokens** : `tokens.css` (à copier dans `packages/tokens/` → exposer en Tailwind preset).
- **Composants visuels** : `components.css` et `app.css` documentent toutes les classes utilisées dans la maquette. Reproduire en React/Tailwind 1:1.
...
- **`tokens.css`** : variables CSS — à porter en Tailwind preset.
- **`components.css`** + **`app.css`** : implémentations CSS de référence pour chaque composant.
